  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (response['status'] == 'success') {
        ...
        final String authToken = user['auth_token'];

        // Pull cloud settings first
        ...
        if (cloudSettings != null && cloudSettings['status'] == 'success') {
          ...
          await prefs.setInt(
            'prep_time',
            int.tryParse(data['prep_time'].toString()) ?? 30,
          );
          await prefs.setInt(
            'rest_time',
            int.tryParse(data['rest_time'].toString()) ?? 30,
          );
          await prefs.setBool(
            'voice_enabled',
            data['voice_enabled'].toString() == '1',
          );
          await prefs.setDouble(
            'feedback_volume',
            double.tryParse(data['feedback_volume'].toString()) ?? 1.0,
          );
          await prefs.setDouble(
            'beeps_volume',
            double.tryParse(data['beeps_volume'].toString()) ?? 1.0,
          );

          debugPrint("CLOUD SETTINGS: Successfully applied to local device.");
        }

        // Pull online history into local DB
        setState(() => _loadingText = 'Syncing workout history...');
        final pulled = await SyncService.pullHistoricalData(userId, authToken);
        debugPrint("LOGIN PULL RESULT: $pulled");
        ...
        setState(() => _message =
            response['message'] ?? 'Login failed. Check credentials.');
      }
    } on TimeoutException {
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text('AI-Powered Workout Assistant',
                      textAlign: TextAlign.center,
                      style:
                  ...
                  TextField(
                      ...
                      decoration:
                          _customInputDecoration('Username', Icons.person)),
                  const SizedBox(height: 16),
                              ...
                              children: [
                                  const SizedBox(
                                      ...
                                      child: CircularProgressIndicator(
                                          strokeWidth: 2.5, color: navyBlue)),
